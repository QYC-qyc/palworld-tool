...
func Schema() []Field {
	fields := []Field{
		...
		{Key: "Difficulty", Label: "难度", Type: TypeEnum, Default: "None",
			...
			Description: "选择预设会自动填入官方倍率；修改任意倍率将自动切回自定义", Group: "世界"},
		stringField("ServerPassword", "进服密码", "",
			"玩家进入服务器需要的密码，留空则无密码", "服务器"),
		stringField("AdminPassword", "管理员密码", "",
			"REST API 使用的管理员密码；保存时自动同步到面板连接设置", "服务器"),
		...
		boolField("RCONEnabled", "启用 RCON", "False",
			"远程控制台端口（官方即将弃用，不建议开启）", "服务器", true),
		intField("RCONPort", "RCON 端口", "25575",
			"RCON 服务监听的 TCP 端口", "服务器", true, 1, 65535),
		{Key: "CrossplayPlatforms", Label: "允许跨玩平台", Type: TypeMulti, Default: "Steam,Xbox,PS5,Mac",
			Description: "选择允许跨平台游玩的平台", Group: "服务器", RequiresRestart: true,
			Options: []FieldOption{opt("Steam", "Steam"), opt("Xbox", "Xbox"), opt("PS5", "PS5"), opt("Mac", "Mac")}},
		...
		rateField("PalCaptureRate", "捕获概率倍率", "1.0", "捕获帕鲁的成功概率倍率", "帕鲁", false, 0.5, 5),
		rateField("PalSpawnNumRate", "帕鲁出现数量倍率", "1.0", "野外帕鲁刷新数量倍率，数值越大服务器负载越高", "帕鲁", false, 0.5, 5),
		{Key: "PalEggDefaultHatchingTime", Label: "巨大蛋孵化时间(小时)", Type: TypeFloat, Default: "72.0",
			Description: "巨大帕鲁蛋所需孵化时长（小时），其余蛋按比例缩放", Group: "帕鲁", Min: fmin(0), Step: fstep(0.1)},
		rateField("PalDamageRateAttack", "帕鲁攻击伤害倍率", "1.0", "帕鲁造成的伤害倍率", "帕鲁", false, 0.1, 5),
		rateField("PalDamageRateDefense", "帕鲁受到伤害倍率", "1.0", "帕鲁承受的伤害倍率，越小越耐打", "帕鲁", false, 0.1, 5),
		rateField("PalAutoHPRegeneRate", "帕鲁自然回血倍率", "1.0", "帕鲁非睡眠状态下生命自然恢复倍率", "帕鲁", false, 0.1, 5),
		rateField("PalAutoHpRegeneRateInSleep", "帕鲁睡眠回血倍率", "1.0", "帕鲁睡眠时生命恢复倍率（官方键名拼写为 Hp）", "帕鲁", false, 0.1, 5),
		rateField("PalStaminaDecreaceRate", "帕鲁耐力消耗倍率", "1.0", "帕鲁耐力消耗速度倍率（官方键名拼写为 Decreace）", "帕鲁", false, 0.1, 5),
		rateField("PalStomachDecreaceRate", "帕鲁饥饿消耗倍率", "1.0", "帕鲁饱食度下降速度倍率（官方键名拼写为 Decreace）", "帕鲁", false, 0.1, 5),
		rateField("PlayerDamageRateAttack", "玩家攻击伤害倍率", "1.0", "玩家造成的伤害倍率", "玩家", false, 0.1, 5),
		rateField("PlayerDamageRateDefense", "玩家受到伤害倍率", "1.0", "玩家承受的伤害倍率，越小越耐打", "玩家", false, 0.1, 5),
		rateField("PlayerAutoHPRegeneRate", "玩家自然回血倍率", "1.0", "玩家非睡眠状态生命自然恢复倍率", "玩家", false, 0.1, 5),
		rateField("PlayerAutoHpRegeneRateInSleep", "玩家睡眠回血倍率", "1.0", "玩家睡眠时生命恢复倍率（官方键名拼写为 Hp）", "玩家", false, 0.1, 5),
		rateField("PlayerStaminaDecreaceRate", "玩家耐力消耗倍率", "1.0", "玩家耐力消耗速度倍率（官方键名拼写为 Decreace）", "玩家", false, 0.1, 5),
		rateField("PlayerStomachDecreaceRate", "玩家饥饿消耗倍率", "1.0", "玩家饱食度下降速度倍率（官方键名拼写为 Decreace）", "玩家", false, 0.1, 5),
		rateField("BuildObjectDamageRate", "建筑受伤倍率", "1.0", "建筑受到攻击时的伤害倍率", "据点", false, 0.5, 3),
		rateField("BuildObjectDeteriorationDamageRate", "建筑损坏速度倍率", "1.0", "建筑无人维护时自然劣化速度倍率", "据点", false, 0.1, 10),
		rateField("CollectionDropRate", "采集掉落倍率", "1.0", "砍伐、采矿等采集产出数量倍率", "经济与掉落", false, 0.5, 5),
		rateField("CollectionObjectHpRate", "采集物血量倍率", "1.0", "树木、矿点等可采集对象生命值倍率，越大越难采集", "经济与掉落", false, 0.5, 3),
		rateField("CollectionObjectRespawnSpeedRate", "采集物重生速度倍率", "1.0", "树木、矿石等重新刷新速度倍率", "经济与掉落", false, 0.5, 5),
		rateField("EnemyDropItemRate", "敌人掉落物倍率", "1.0", "击败敌人/帕鲁时掉落物品数量倍率", "经济与掉落", false, 0.5, 5),
		rateField("MonsterFarmActionSpeedRate", "牧场生产速度倍率", "1.0", "牧场帕鲁产出物品的速度倍率", "帕鲁", false, 0.1, 10),
		rateField("ItemWeightRate", "物品重量倍率", "1.0", "物品重量倍率，越小可携带越多", "经济与掉落", false, 0.1, 5),
		rateField("ItemCorruptionMultiplier", "物品腐坏速度倍率", "1.0", "食物腐坏速度倍率", "经济与掉落", false, 0.1, 10),
		rateField("EquipmentDurabilityDamageRate", "装备耐久损耗倍率", "1.0", "武器、防具耐久度下降速度倍率", "经济与掉落", false, 0.1, 5),
		intField("SupplyDropSpan", "空投补给间隔(分)", "180",
			"空投和陨石出现的间隔（分钟）", "世界", false, 0, 0),
		boolField("bEnableFastTravel", "启用快速旅行", "True", "允许使用传送点快速旅行", "世界", false),
		boolField("bEnableFastTravelOnlyBaseCamp", "仅据点间快速旅行", "False", "限制快速旅行只能在据点之间进行", "世界", false),
		boolField("bIsStartLocationSelectByMap", "允许选择出生地点", "True", "新玩家可在地图上选择初始出生区域", "世界", false),
		...
		// ---- 功能 ----
		boolField("bIsPvP", "PvP 模式", "False", "开启后玩家之间可以互相伤害", "战斗", false),
		boolField("bHardcore", "硬核模式", "False", "角色死亡后无法复活", "战斗", true),
		boolField("bPalLost", "死亡永久失去帕鲁", "False", "玩家死亡将永久失去其帕鲁", "战斗", false),
		boolField("bCanPickupOtherGuildDeathPenaltyDrop", "可拾取他公会死亡掉落", "False",
			"允许拾取其他公会成员死亡惩罚掉落的物品", "战斗", false),
		boolField("bEnableFriendlyFire", "友军伤害", "False", "同公会/队友攻击也会造成伤害", "战斗", false),
		boolField("bCharacterRecreateInHardcore", "硬核模式可重建角色", "False",
			"硬核模式死亡后允许重建角色", "战斗", false),
		...
		boolField("bExistPlayerAfterLogout", "离线角色留在世界", "False", "玩家登出后角色仍停留在游戏世界中", "玩家", false),
		boolField("bShowPlayerList", "ESC 菜单显示玩家列表", "True", "暂停/ESC 菜单中显示在线玩家列表", "公会", false),
		boolField("bIsShowJoinLeftMessage", "显示进/退服消息", "True", "玩家加入或离开时在聊天框提示", "服务器", false),
		boolField("bEnableVoiceChat", "启用语音聊天", "False", "启用游戏内置语音聊天", "公会", false),
		boolField("bBuildAreaLimit", "限制快速旅行点附近建造", "False", "只能在快速旅行点/据点附近建造", "据点", false),
		boolField("bAllowGlobalPalboxImport", "允许跨界帕鲁终端导入", "False", "允许从其他存档/跨界帕鲁终端导入帕鲁", "公会", false),
		boolField("bAllowGlobalPalboxExport", "允许跨界帕鲁终端导出", "False", "允许把帕鲁导出到跨界帕鲁终端", "公会", false),
		...
		boolField("bEnableBuildingPlayerUIdDisplay", "显示建筑创建者ID", "False",
			"在建筑上显示创建者的玩家 ID", "服务器", false),
		boolField("EnablePredatorBossPal", "启用掠食者头目帕鲁", "True",
			"允许野生掠食者头目出现", "帕鲁", false),
		intField("DropItemMaxNum", "掉落物最大数量", "3000",
			"世界中同时存在的掉落物上限", "经济与掉落", false, 0, 0),
		boolField("bActiveUNKO", "启用排泄物掉落", "False", "启用帕鲁排泄物掉落机制", "经济与掉落", false),
		intField("DropItemMaxNum_UNKO", "排泄物上限", "100", "世界中同时存在的排泄物上限", "经济与掉落", false, 0, 0),
		...
		{Key: "RespawnPenaltyDurationThreshold", Label: "复活惩罚阈值(秒)", Type: TypeFloat, Default: "0",
			Description: "应用复活时间倍率的连续死亡间隔阈值（秒）", Group: "玩家", Min: fmin(0)},
		...
		boolField("bAllowEnhanceStat_Weight", "允许强化负重", "True", "允许玩家分配点数到负重", "玩家", false),
		boolField("bAllowEnhanceStat_WorkSpeed", "允许强化工作速度", "True", "允许玩家分配点数到工作速度", "玩家", false),
		boolField("bEnableAimAssistPad", "手柄瞄准辅助", "True", "启用手柄瞄准辅助", "玩家", false),
		boolField("bEnableAimAssistKeyboard", "键盘瞄准辅助", "False", "启用键盘鼠标瞄准辅助", "玩家", false),
		stringField("AdditionalDropItemWhenPlayerKillingInPvPMode", "PvP击杀掉落物品ID", "PlayerDropItem",
			"PvP模式击杀玩家时额外掉落的物品ID", "玩家"),
		intField("AdditionalDropItemNumWhenPlayerKillingInPvPMode", "PvP击杀掉落数量", "1",
			"PvP模式击杀玩家时额外掉落物品数量", "玩家", false, 0, 100),
		boolField("bAdditionalDropItemWhenPlayerKillingInPvPMode", "启用PvP击杀掉落", "False",
			"PvP模式击杀玩家时掉落额外物品", "玩家", false),
