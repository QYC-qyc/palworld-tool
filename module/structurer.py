...
def _extract_instance_id(key):
    """从 CharacterSaveParameterMap 的 key 中安全提取 InstanceId（可能是 Guid 对象）。"""
    node = key.get("InstanceId")
    if not node:
        return None
    ...
    for uid, instance_id, c in uid_character:
        if c.get("IsPlayer") and c["IsPlayer"]["value"]:
            ...
            if not c.get("OwnerPlayerUId"):
                continue
            pals.append(Pal(c, instance_id=instance_id).to_dict())
